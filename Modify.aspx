...
CommandText='<%# "SELECT * FROM 客戶資料表 WHERE Serial = ?" %>'
Debug="true"
>
  <Parameters>
...
'讀取原始資料
   CustId.text=DataSet1.FieldValue("CustId", Nothing)
   CustName.text=DataSet1.FieldValue("CustName", Nothing) 
   CustTelPhone.text=DataSet1.FieldValue("CustTelPhone", Nothing)	
   CustMobile.text=DataSet1.FieldValue("CustMobile", Nothing)
   CustEmail.text=DataSet1.FieldValue("CustEmail", Nothing)
   CustAddress.text=DataSet1.FieldValue("CustAddress", Nothing)
   Contect.text=DataSet1.FieldValue("Contect", Nothing)
...
Sub Button1Cilck(ByVal sender As Object, ByVal e As EventArgs)
  ...
  Dim Url As String
    ...
    Url="Save.aspx?Serial="+DataSet1.FieldValue("Serial", Nothing)
	
	Url=Url+"&CustId="+CustId.text
...
<form name='form' method='POST' runat='server'>
  <table width="423" border="1">
    <tr>
      <td colspan="2" bgcolor="#6699FF"><span class="style1">修改客戶資料</span></td>
    </tr>
    <tr>
      <td width="92">客戶編號</td>
      <td width="315"><asp:TextBox ID="CustId" runat="server" />
      <input type="hidden" name="Serial" id="Serial" /></td>
    </tr>
    <tr>
      <td>姓名</td>
  ...
  <p>
    <asp:Button ID="Button1" runat="server" Text="修改" OnClick="Button1Cilck" />    
